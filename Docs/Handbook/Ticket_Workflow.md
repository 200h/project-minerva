...
Implementation is complete enough for review and includes the required implementation report and validation evidence.

### Complete

The change is merged, acceptance criteria are satisfied, documentation is current, and follow-up items are captured separately.
...
Move the Markdown file between state directories. Do not duplicate it. Update status metadata and retain the same identifier and filename.
...
After merge, move the ticket to Complete and record:
